...
if args.blast_file and not args.otu_file:
    print ('\n\nERROR: --otu_file.txt must be given with --blast_file')
    print ('Please provide otu file used to create blast sequences when giving pre-computed blast results.')
    sys.exit()
...
if not args.blast_file and not args.otu_file:
    if args.verbose:
    ...
    if current_query != sequence_id: #INITIATION of new query
        if bool(current_best_hits) and current_query: #ENSURE A SET OF BLASTS TO PARSE
            ...
            for key,value in best_blast_dictionary.items():
                ...
                if args.verbose:
                    print (pkey, taxonomy_dictionary[value[1]])
            ...
            if args.verbose:
                print ('Taxonomy Assignment for', current_query, '= ', ':'.join(best_level_taxonomy))
...
os.system("sort -k2 " + outdir + "taxonomy_assignment_qiime_format.txt > " + outdir + "taxonomy_assignment_qiime_format2.txt && mv " + outdir + "taxonomy_assignment_qiime_format2.txt " + outdir + "taxonomy_assignment_qiime_format.txt")
os.system("sort -t: -k4 " + outdir + "taxonomy_assignment_per_sequence.txt > " + outdir + "taxonomy_assignment2.txt && mv " + outdir + "taxonomy_assignment2.txt " + outdir + "taxonomy_assignment_per_sequence.txt")
...
def make_otu_table(otu_file,taxonomy_file):
    ...
    sp.communicate()
    os.system('mv otu_table.biom ' + outdir)
